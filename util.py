...
def is_fasta(seq):
    ...
    if not seq.name:
        ...
        return False
    if -1 != seq.name.find('>'):
        error_info = 'Warming, sequence ' + str(seq.no) + ' name has > character. The sequence name is ' + str(seq.name)
        print(error_info)
        return True
    if 0 == seq.length:
...
def write_gistsvm_vec(vecs, write_file):
    with open(write_file, 'wb') as f:
        ...
        for i in range(len_vec):
            f.write(bytes("\t" + "feature_" + str(i), encoding="UTF-8"))
        for i, vec in enumerate(vecs):
            f.write(bytes("\n", encoding="UTF-8"))
            f.write(bytes(str(i), encoding="UTF-8"))
            for j in range(len_vec):
                f.write(bytes("\t" + str(vec[j]), encoding="UTF-8"))
...
def write_gistsvm_class(labels, write_file):
    with open(write_file, 'wb') as f:
        ...
        for i, label in enumerate(labels):
            f.write(bytes("\n", encoding="UTF-8"))
            f.write(bytes(str(i) + "\t" + str(label), encoding="UTF-8"))
